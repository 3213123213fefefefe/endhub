return function(H)
    ...
    local function addRarityDropdown(group, rarity)
        ...
        -- Some UI builds invoke a Multi dropdown callback during construction.
        -- Capture the persisted selection first so that initialization cannot erase it.
        local persisted = selectedFor(rarity)
        ...
        if Options[id] then Options[id]:SetValue(persisted) end
    end
