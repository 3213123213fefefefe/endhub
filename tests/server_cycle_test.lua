...
test("only a complete empty lap hops; collecting repeats the route", function()
    ...
    t.R.Bootstrap(); t.advance(1)
    t.H.Farm.Step(); t.advance(0); equal(t.routeTeleports, 1)
    ...
    t.advance(1.1); t.H.Farm.Step(); t.advance(0); equal(t.routeTeleports, 2)
    t.H.State.LootPickupSerial = 1
    t.advance(1.1); t.H.Farm.Step(); t.advance(1)
    equal(#t.teleports, 0); equal(t.routeTeleports, 3)
    t.H.Farm.Step(); t.advance(0); equal(t.routeTeleports, 4)
    t.advance(1.1); t.H.Farm.Step(); t.advance(1)
...
test("unified UI preserves saved sell filters despite dropdown initialization callbacks", function()
    local t = context()
    ...
    equal(#names, 10); equal(names[1], "Movement"); equal(names[4], "Farm"); equal(names[10], "Debug")
    equal(filters.Common.Trinket, true)
    assert(t.H.UI.Tabs.Visuals); assert(t.H.UI.Tabs.Movement)
end)
